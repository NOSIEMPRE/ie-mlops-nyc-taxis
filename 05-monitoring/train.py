...
def train_and_log(X_train_dicts, y_train, X_val_dicts, y_val):
    ...
    with mlflow.start_run() as run:
        ...
        # Get local artifact path for app.py fallback (avoids MLflow server fetch hang)
        artifact_uri = mlflow.get_artifact_uri("model")
        model_path = None
        if "mlflow-artifacts:" in artifact_uri:
            # mlflow-artifacts:/1/models/m-xxx/artifacts -> mlartifacts/1/models/m-xxx/artifacts
            model_path = "mlartifacts" + artifact_uri.split("mlflow-artifacts:")[-1]
        elif artifact_uri.startswith("file:"):
            model_path = artifact_uri.replace("file://", "").replace("file:", "")
        ...
        print("🖥  View MLflow UI: http://localhost:5001")
