...
├── simulate.py     # Simulates live predictions → data/predictions.csv
├── monitor.py      # Generates drift report → monitoring_report.html
├── test_api.py     # Basic health + predict tests
└── README.md       # This guide
```

*Generated by scripts: `run_id.txt`, `model_path.txt`, `data/`, `mlflow.db`, `mlartifacts/`, `monitoring_report.html`*
...
Open [http://localhost:5001](http://localhost:5001) (leave running).
...
- **MLflow UI**: Inspect all runs, parameters, metrics, and artifacts at [http://localhost:5000](http://localhost:5000)
