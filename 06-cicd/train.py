...
CI/CD version: runs without MLflow server, saves model to models/model/.
Aligns with 05-monitoring: same features (PU_DO, trip_distance), same params.
"""
...
class DictVectorizerWrapper(BaseEstimator, TransformerMixin):
    """Sklearn-compatible wrapper around DictVectorizer (matches 05-monitoring)."""
...
def load_data(limit: int = 100_000) -> pd.DataFrame:
    """Load and filter NYC taxi data (same logic as 05-monitoring)."""
    print("Loading data...")
...
def prepare_features(df: pd.DataFrame):
    """Create feature dicts and target (PU_DO, trip_distance; matches 05-monitoring)."""
...
def train_and_log(X_train, y_train, X_val, y_val):
    """Train model, log to MLflow, and save to models/model/ for deployment."""
